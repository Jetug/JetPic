...
fun Context.getDirectoryGroup(path: String): String{
    val settingsDb: FolderSettings = runBlocking { IOScope.async { getFolderSettings(path) }.await() }
...
private fun Context.readSettingsFromFile(path: String): FolderSettings{
    ...
    if(settings == null){
        ...
        if (hasStoragePermission) launchIO {
            writeSettingsToFile(path, settings)
        }
    }
